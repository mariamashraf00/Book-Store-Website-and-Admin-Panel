...
class Books
{
    ...
    public static function update_copies($isbn, $copies) {
        $instance = ConnectionDb::getInstance();
        $conn = $instance->getConnection();
        $book_update = $conn->prepare("UPDATE `books` SET `copies`=? WHERE `isbn`=?;");
        $book_update->bind_Param(
            'is',
            $copies,
            $isbn
    ...
    public static function get_count()
    {
        ...
        if ($ret_all->execute()) {
            return  mysqli_num_rows(mysqli_stmt_get_result($ret_all));
        } else {
            return 0;
        }
    }
    ...
    public static function retrive_limit($num)
    {
        // returns object of rows get each by "$result->fetch_assoc();"
        $instance = ConnectionDb::getInstance();
        $conn = $instance->getConnection();
        $ret_all = $conn->prepare("SELECT * FROM `books` LIMIT $num,20;");
        if ($ret_all->execute()) {
