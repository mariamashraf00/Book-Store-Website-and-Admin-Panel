...
class Customer
{
    ...
    public static function retrieve_history_by_username($username)
    {
        // return associative array
        $instance = ConnectionDb::getInstance();
        $conn = $instance->getConnection();
        $retrive_id = $conn->prepare("SELECT * FROM books,orders,order_items,`customers` WHERE customer_username=username AND order_id=id AND book_isbn=isbn AND username = ? ;");
        $retrive_id->bind_param('s', $username);
        if ($retrive_id->execute()) {
